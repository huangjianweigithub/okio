...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable {
  ...
  public UnsafeCursor readUnsafe(UnsafeCursor unsafeCursor) {
    if (unsafeCursor.buffer != null) throw new IllegalStateException("already acquired");
   ...
   * <h3>Warnings</h3>
   *
   * <p>Most application developers should avoid this API. Those that do should respect these
   * warnings.
   *
  ...
  public static final class UnsafeCursor implements Closeable {
    ...
    public void resizeBuffer(long newSize) {
      ...
      if (newSize < buffer.size) {
        if (newSize < 0) {
          throw new IllegalArgumentException("newSize: " + newSize + " < size: " + buffer.size);
        }
        if (offset > newSize) {
          offset = newSize;
        }
        // Shrink the buffer by either shrinking segments or removing them.
        for (long bytesToSubtract = buffer.size - newSize; bytesToSubtract > 0; ) {
        ...
        for (long bytesToAdd = newSize - buffer.size; bytesToAdd > 0; ) {
